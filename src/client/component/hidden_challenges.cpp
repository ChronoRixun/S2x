	...
	namespace
	{
			...
			const std::uint16_t target, const bool bitfield)
		{
			...
			const auto result = demonware::achievement_store::mutate(name,
				...
				{
					previous_progress = record.progress;
					// A chapter record finished under an old target (including the old
					// unlock command's 1/1) represents every chapter, not a partial mask.
					// Normalize only for this write; unfinished records keep their bits.
					const auto stored_progress = bitfield && record.progress_target != target &&
						record.progress_target > 0 && record.progress >= record.progress_target &&
						(record.completion_timestamp || record.fulfilled_times > 0)
						? target : record.progress;
					updated_progress = bitfield
						? static_cast<std::uint16_t>((stored_progress & target) | progress)
						: std::max(stored_progress, progress);

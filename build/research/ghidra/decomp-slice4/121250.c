// image offset 0x121250 address 7ff7a32b1250

/* WARNING: Function: __security_check_cookie replaced with injection: security_check_cookie */

undefined8 FUN_7ff7a32b1250(longlong param_1)

{
  undefined4 *puVar1;
  char cVar2;
  int iVar3;
  undefined8 uVar4;
  longlong lVar5;
  uint *puVar6;
  uint *puVar7;
  longlong lVar8;
  float fVar9;
  undefined1 auStack_58 [32];
  undefined1 local_38 [32];
  ulonglong local_18;
  
  local_18 = DAT_7ff7a4152aa8 ^ (ulonglong)auStack_58;
  if (((int)((longlong)(*(ulonglong *)(param_1 + 0x48) - *(ulonglong *)(param_1 + 0x50)) >> 4) == 2)
     && (*(ulonglong *)(param_1 + 0x50) < *(ulonglong *)(param_1 + 0x48))) {
    iVar3 = FUN_7ff7a3468d90(param_1);
    if (iVar3 != 0) {
      puVar6 = *(uint **)(param_1 + 0x50);
      puVar7 = *(uint **)(param_1 + 0x48);
      if ((puVar6 + 4 < puVar7) && ((puVar6[4] & 0xf) - 3 < 2)) goto LAB_7ff7a32b12ce;
    }
  }
  FUN_7ff7a34731a0(param_1,&DAT_7ff7a3cbd562);
  puVar6 = *(uint **)(param_1 + 0x50);
  puVar7 = *(uint **)(param_1 + 0x48);
LAB_7ff7a32b12ce:
  if (puVar6 < puVar7) {
    fVar9 = (float)FUN_7ff7a3469aa0(param_1,puVar6);
    puVar6 = *(uint **)(param_1 + 0x50);
    puVar7 = *(uint **)(param_1 + 0x48);
    iVar3 = (int)fVar9;
  }
  else {
    iVar3 = 0;
  }
  if (puVar6 + 4 < puVar7) {
    uVar4 = FUN_7ff7a3469740(param_1,puVar6 + 4,0);
  }
  else {
    uVar4 = 0;
  }
  FUN_7ff7a39c90a0(local_38);
  cVar2 = FUN_7ff7a3408180(iVar3,uVar4,local_38,1);
  puVar1 = *(undefined4 **)(param_1 + 0x48);
  puVar1[2] = (uint)(cVar2 != '\0');
  *puVar1 = 1;
  *(undefined4 **)(param_1 + 0x48) = puVar1 + 4;
  lVar5 = FUN_7ff7a39c9020(local_38);
  if (lVar5 == 0) {
    puVar1 = *(undefined4 **)(param_1 + 0x48);
    *puVar1 = 0;
    *(undefined4 **)(param_1 + 0x48) = puVar1 + 4;
  }
  else {
    lVar8 = -1;
    do {
      lVar8 = lVar8 + 1;
    } while (*(char *)(lVar5 + lVar8) != '\0');
    FUN_7ff7a3294620(param_1,lVar5);
  }
  return 2;
}


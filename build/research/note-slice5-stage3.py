...
edit('src/client/component/hq_vendor_globals.hpp',lambda s:s.replace('inline const std::uintptr_t','inline constexpr std::uintptr_t').replace('_g,', ','))
edit('src/client/component/hq_native.cpp',lambda s:s.replace('for (const auto address : vendor_globals)\n\t\t\t{','for (const auto offset : vendor_globals)\n\t\t\t{\n\t\t\t\tconst auto address = 0x0_g + offset;').replace('address - 0x0_g, raw','offset, raw').replace('std::memset(transaction, 0, 25);','if (transaction) std::memset(transaction, 0, 25);').replace('''						console::info("[HQ payroll] delivered persisted completion push\\n");
					}
...
edit('src/client/component/mail_guard.cpp',lambda s:s.replace('// at least one message so the array is allocated, but a reply that fails, arrives','// at least 14 messages so the array is allocated, but a reply that fails, arrives').replace('''			console::info("[HQ mail] %s controller=''','''			// Capture initial polls and changes without flooding the per-frame console.
			...
			console::info("[HQ mail] %s controller='''))
p=Path('build/research/slice5-mail.md');p.write_text('''# Slice 5 Mail
...
Stage3 also fixes diagnostic RVA resolution to occur after image mapping and

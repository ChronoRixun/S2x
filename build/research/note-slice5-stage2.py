...
p=Path('build/research/hq-tests/tests.cpp');s=p.read_text().replace(' // Native catalog read-side order',' {\n // Native catalog read-side order').replace(' byte_buffer no_terminator',' }\n byte_buffer no_terminator');p.write_bytes(s.replace('\n','\r\n').encode())
p=Path('build/research/slice5-quartermaster.md');p.write_text('''# Slice 5 Quartermaster
